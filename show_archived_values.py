...
parser.add_argument('--page-size', type=int, default=2,
                    help='Page size to use when retrieving values (default 2)')
parser.add_argument('--max-values', type=int, default=20,
                    help='Max no. of values to show (default 20)')
...
print('Archived values:')
count = 0
for pv in archive.list_parameter_values(args.parameter,
                                        page_size=args.page_size):
    t = datetime.fromtimestamp(pv.generation_time.timestamp(), timezone.utc)
    print(f'    {t}: value={pv.eng_value}')
    count += 1
